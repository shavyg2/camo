...
class Document extends BaseDocument {
    ...
    save() {
        ...
        return Promise.all(preValidatePromises).then(function() {
            ...
            var data = prevData[0];
            return (this._DB || DB()).save(that.collectionName(), that._id, data);
        }).then(function(id) {
            if (that._id === null) {
                that._id = id;
            }
    ...
    delete() {
        ...
        return Promise.all(preDeletePromises).then(function() {
            return (this._DB || DB()).delete(that.collectionName(), that._id);
    ...
    static deleteOne(query) {
        return (this._DB || DB()).deleteOne(this.collectionName(), query);
    }
    ...
    static deleteMany(query) {
        ...
        return (this._DB || DB()).deleteMany(this.collectionName(), query);
    }
    ...
    static findOne(query, options) {
        ...
        return (this._DB || DB()).findOne(this.collectionName(), query)
        .then(function(data) {
            if (!data) {
                return null;
            }
    ...
    static findOneAndUpdate(query, values, options) {
        ...
        return (this._DB || DB()).findOneAndUpdate(this.collectionName(), query, values, options)
        .then(function(data) {
            if (!data) {
                return null;
            }
    ...
    static findOneAndDelete(query, options) {
        ...
        return (this._DB || DB()).findOneAndDelete(this.collectionName(), query, options);
    }
    ...
    static find(query, options) {
        ...
        return (this._DB || DB()).find(this.collectionName(), query, options)
        .then(function(datas) {
    ...
    static count(query) {
        var that = this;
        return (this._DB || DB()).count(this.collectionName(), query);
    }
    ...
    static createIndexes() {
        ...
        _.keys(instance._schema).forEach(function(k) {
            if (instance._schema[k].unique) {
                (this._DB || DB()).createIndex(that.collectionName(), k, {unique: true});
            }
        });
    ...
    static clearCollection() {
        return (this._DB || DB()).clearCollection(this.collectionName());
    }
