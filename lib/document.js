...
class Document extends BaseDocument {
    ...
    save() {
        ...
        return Promise.all(preValidatePromises).then(function() {
            ...
            var data = prevData[0];
            return (this_DB || DB()).save(that.collectionName(), that._id, data);
        }).then(function(id) {
            if (that._id === null) {
                that._id = id;
            }
    ...
    delete() {
        ...
        return Promise.all(preDeletePromises).then(function() {
            return (this_DB || DB()).delete(that.collectionName(), that._id);
    ...
    static deleteOne(query) {
        return (this_DB || DB()).deleteOne(this.collectionName(), query);
    }
    ...
    static deleteMany(query) {
        ...
        return (this_DB || DB()).deleteMany(this.collectionName(), query);
    }
    ...
    static findOne(query, options) {
        ...
        return (this_DB || DB()).findOne(this.collectionName(), query)
        .then(function(data) {
            if (!data) {
                return null;
            }
    ...
    static findOneAndUpdate(query, values, options) {
        ...
        return (this_DB || DB()).findOneAndUpdate(this.collectionName(), query, values, options)
        .then(function(data) {
            if (!data) {
                return null;
            }
    ...
    static findOneAndDelete(query, options) {
        ...
        return (this_DB || DB()).findOneAndDelete(this.collectionName(), query, options);
    }
    ...
    static find(query, options) {
        ...
        return (this_DB || DB()).find(this.collectionName(), query, options)
        .then(function(datas) {
    ...
    static count(query) {
        var that = this;
        return (this_DB || DB()).count(this.collectionName(), query);
    }
    ...
    static createIndexes() {
        ...
        _.keys(instance._schema).forEach(function(k) {
            if (instance._schema[k].unique) {
                (this_DB || DB()).createIndex(that.collectionName(), k, {unique: true});
            }
        });
    ...
    static clearCollection() {
        return (this_DB || DB()).clearCollection(this.collectionName());
    }
